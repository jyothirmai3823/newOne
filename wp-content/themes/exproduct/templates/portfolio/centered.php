		...
		<div class="image">
			...
			if ( $exproduct_portfolio_post_type == 'video' ) : ?>
				...
				if ( $exproduct_portfolio_video_href != '' ) :
					...
					?>
					<div class="controls">
						<div class="big-view"><a href="<?php echo esc_url($exproduct_portfolio_video_href.'?width='.esc_attr($exproduct_portfolio_video_width).'&amp;height='. esc_attr($exproduct_portfolio_video_height)) ?>" class="fancybox" data-rel="gallery[pp_video_<?php echo esc_attr($post->ID); ?>]"  ><span class="icon icon-Media"></span></a></div>
					</div>
				<?php
				endif;
				?>
