		...
		<div class="row">
			...
			<div class="<?php if ( $exproduct_layout == 1 ) : ?>col-lg-12 col-md-12<?php else : ?>col-lg-9 col-md-8<?php endif; ?> col-sm-12 col-xs-12 left-column sidebar-type-<?php echo esc_attr($exproduct_layout); ?>">
				...
				<?php
					...
					$exproduct_portfolio_perpage = exproduct_get_option('portfolio_settings_perpage');
					if ( is_numeric( $exproduct_portfolio_perpage ) && $exproduct_portfolio_perpage > 0 ) {
						$exproduct_archive_perpage = $exproduct_portfolio_perpage;
					}
					else {
						$exproduct_archive_perpage = -1;
					}
					...
					$args = array(
								...
								'paged' => $paged,
								'posts_per_page' => $exproduct_archive_perpage
							);
					...
					if ( $wp_query->have_posts() ) : ?>
						...
						while ( $wp_query->have_posts() ) :
							...
							if ( $exproduct_portfolio_type == 'type_without_icons' || $exproduct_portfolio_type == 'type_without_space' ) : ?>
								<?php $exproduct_no_space_class = $exproduct_portfolio_type == 'type_without_space' ? 'pix-no-space' : ''; ?>
									<div class="<?php echo esc_attr($exproduct_add_class_port_col); ?> item <?php echo esc_attr($exproduct_no_space_class); ?> <?php echo esc_attr($exproduct_portfolio_css_animation); ?> <?php echo esc_attr($exproduct_cat_slugs); ?>" id="post-<?php echo esc_attr(get_the_ID()); ?>">
