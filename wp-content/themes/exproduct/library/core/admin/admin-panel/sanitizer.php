<?php
	...
	function exproduct_sanitize_text( $input ) {
		return wp_kses_post( force_balance_tags( esc_attr($input) ) );
	}

	function exproduct_sanitize_checkbox( $input ) {
		if ( $input == '1' ) {
			return '1';
		} else {
			return '';
	...
	function exproduct_sanitize_per_page( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function exproduct_sanitize_absinteger( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function exproduct_sanitize_email( $email ) {
		if (is_email( $email ) ) {
			return $email;
		} else {
			return '';
	...
	function exproduct_sanitize_layout( $value ) {
	...
	function exproduct_sanitize_loader( $value ) {
	...
	function exproduct_sanitize_onoff( $value ) {
	...
	function exproduct_sanitize_sidebar_blog_type( $value ) {
	...
	function exproduct_sanitize_sidebar_blog_content( $value ) {
	...
	function exproduct_sanitize_header_type( $value ) {
	...
	function exproduct_sanitize_overlay_opacity( $value ) {
	...
	function exproduct_sanitize_portfolio_type( $value ) {
	...
	function exproduct_sanitize_portfolio_perrow( $value ) {
	...
	function exproduct_sanitize_sidebar_portfolio_type( $value ) {
		if ( ! in_array( $value, array( '1', '2', '3' ) ) ) {
			$value = '2';
		}
	...
	function exproduct_sanitize_sidebar_portfolio_content( $value ) {
	...
	function exproduct_sanitize_animation( $value ) {
	...
	function exproduct_sanitize_footer_block($input) {
		$valid = exproduct_get_staticblock_option_array();
