<?php 
	...
	function exproduct_js_vars(){
		
		$vars = '';

		$_js = apply_filters('exproduct_js_vars',$vars);
	...
	function exproduct_css_vars(){
		
		$css = '';
		$header_color  = exproduct_get_option('header_all_color');
		$footer_color  = exproduct_get_option('footer_all_color');
		if ($footer_color){
	...
	function exproduct_get_theme_header($headerType = 'header1'){
	...
	function exproduct_load_block($block_name){
	...
	function exproduct_woo_get_page_id(){
	...
	function exproduct_checkAvailableJsToPage($types){
	...
	function exproduct_get_staticblock_content($blockId){
		...
		if (is_array($blockId)){
			...
			foreach($blockId as $bId){
				if ($bId == 'global'){
					$bId = exproduct_get_option('footer_block');
				}
				$_block = get_post($bId);
	...
	function exproduct_get_staticblock_option_array(){
