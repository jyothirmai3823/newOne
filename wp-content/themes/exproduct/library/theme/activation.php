...
function activate_license($envatoCode, $option_name, $option_name_code) {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			$message = __( 'An error occurred, please try again.', 'exproduct' );
		}
        ...
        if ( 1 ) {
            echo "
                <div style='display:block' class='update-nag'>
                    <h3 class='pix_notice_title'>" . esc_html__("Theme activation", "exproduct") . "</h3>
                    <p>" . esc_html__("Your code", "exproduct") . "<input style='width:280px;margin:0 10px' type='text' name='pix_code' data-activationtheme='1' value='{$envatoCode}'><a data-activationtheme='1' class='button button-primary activation-theme'>" . esc_html__('Activation', 'exproduct') . "</a></p><p class='activated' style='display:none'>" . esc_html__("Theme activated", "exproduct") . "</p>
                </div>
            ";
        }
    }
...
(function($) {
	...
	jQuery('a.delete_key').on("click", function(e) {
		...
		jQuery.post( pixAjax.url, data, function(response) {
			if(response == 1){
				$input.val('');
				alert('<?php echo __('The key deactivated', 'exproduct');  ?>');
			}else{
				alert('<?php echo __('Deactivate the key failed', 'exproduct');  ?>');
			}
		});
	});
