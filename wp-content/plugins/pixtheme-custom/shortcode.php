<?php
function rettic_share_buttons($atts, $content=NULL){

    extract(shortcode_atts(array(
        'class' => '',
        'title' => esc_html__('Share this post','rettic'),
        'post_type'=>'',
    ), $atts));
    ...
    if( $post_type == '' ){
        $out='
            <div class="footer-meta btn-social">
					...
					<a  href="http://pinterest.com/pin/create/button/?url='.$permalink.'&amp;media='.$image[0].'&amp;description='.$post_title.'" target="_blank"><i class="fa fa-pinterest"></i></a>
					<a  href="https://twitter.com/share?url='.$permalink.'&text='.$post_title.'" title="'.__('Twitter', 'rettic').'" target="_blank"><i class="fa fa-twitter"></i></a>
					<a  href="http://www.facebook.com/sharer.php?u='.$permalink.'&amp;images='.$image[0].'" title="'.__('Facebook', 'rettic').'" target="_blank"><i class="fa fa-facebook"></i></a>
					<a  class="latest" href="http://plus.google.com/share?url='.$permalink.'&title='.$post_title.'" title="'.__('Google +', 'rettic').'" target="_blank"><i class="fa fa-google"></i></a>
			</div>
			';
...
add_shortcode('share', 'rettic_share_buttons');